...
import { StructuredOutputParser } from "@langchain/core/output_parsers"; //received json and go straight to the output parser
import { MongoClient } from "mongodb";
import { MongoDBAtlasVectorSearch } from "@langchain/mongodb";
import { z } from "zod"; //for validation type safety
...
const llm = new ChatOpenAI({
  ...
  temperature: 0.7 //0 is very strict, 1 is very creative (because we want to be creative on creating dummy data)
});

//Employee Schema, supposedly we create a new file for this called employee-schema.ts in folder named schemas
...
//this function is to generate synthetic data for the database
...
//when it comes to RAG, we have data that we want to vectorize and create embeddings for that and allows us to search for similar data
//example: cats and dogs, they are located in the same area in the vector space

//we want to create employee summary for each employee based on the fields created by the AI
...
async function seedDatabase(): Promise<void> {
  try {
    ...
    const db = client.db("hr_database"); //create a new database called hr_database
    const collection = db.collection("employees"); //create a new collection called employees

    await collection.deleteMany({}); //for testing purposes, delete all the data in the collection

    const syntheticData = await generateSyntheticData();

    console.log("Generated synthetic data:", syntheticData);
    ...
    for (const record of recordsWithSummaries) {
      await MongoDBAtlasVectorSearch.fromDocuments(
        ...
        {
          collection, //tell the vector search to save the embeddings in the database
          indexName: "vector_index", // Name of the vector search index in MongoDB
          textKey: "embedding_text", // Where the original text is stored
          embeddingKey: "embedding" // Where the vector representation is stored
        }
      );
